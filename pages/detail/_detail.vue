    ...
    <main class="main">
      <div class="layout-left">
        <div class="page-layout">
          <breadcrumb v-if="!isNonSeoArticle" :info="newInfo"></breadcrumb>
          <article class="article" v-if="newInfo">
            <h1 class="article-title">{{ newInfo.name }}</h1>
            <div class="news-author" v-if="!isNonSeoArticle">
              <nuxt-link
...
export default {
  ...
  computed: {
    ...
    // 2026-09-09确认），用它区分"真SEO文章"和"投放落地页"。只在明确拿到
    // is_seo=false/0时才判定为非SEO文章，字段缺失(undefined/null，比如
    // 接口异常没返回)时不算——避免因为接口临时缺字段就把正常SEO文章也
    // 一起隔离/隐藏了。同时用于noindex和隐藏面包屑/作者时间/封面图
    isNonSeoArticle() {
      return this.newInfo && (this.newInfo.is_seo === false || this.newInfo.is_seo === 0);
    }
  },
  head() {
